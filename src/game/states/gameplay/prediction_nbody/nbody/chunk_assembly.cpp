...
#include "game/states/gameplay/prediction_nbody/gameplay_prediction_cache_internal.h"
...
namespace Game
{
    ...
    namespace
    {
        bool valid_planned_chunk_metadata(const OrbitPredictionPublishedChunk &published_chunk)
        {
            return published_chunk.includes_planned_path &&
                ...
                const bool use_dense_chunk_samples)
        {
            ...
            if (!valid_planned_chunk_metadata(published_chunk) ||
            ...
            const bool use_dense_chunk_samples)
    {
        ...
        double previous_chunk_t0_s = std::numeric_limits<double>::lowest();
        std::size_t total_chunk_segment_count = 0;
        std::size_t total_chunk_sample_count = 0;
        double total_chunk_duration_s = 0.0;
        OrbitPredictionAdaptiveStageDiagnostics planned_stage_diagnostics{};
        bool planned_stage_diagnostics_valid = false;
        const auto &planned_inertial_segments = solver.planned.trajectory_segments_inertial;
        ...
        for (const OrbitPredictionPublishedChunk &published_chunk : published_chunks)
        {
            ...
            std::vector<orbitsim::TrajectorySegment> clipped_inertial_segments{};
            if (!slice_trajectory_segments_from_cursor(planned_inertial_segments,
                                                       published_chunk.t0_s,
                                                       published_chunk.t1_s,
                                                       segment_cursor,
                                                       clipped_inertial_segments))
            {
                update_derived_diagnostics(diagnostics, display, PredictionDerivedStatus::ContinuityFailed);
                return false;
            }
            if (clipped_inertial_segments.empty() || !validate_trajectory_segment_continuity(clipped_inertial_segments))
            {
                update_derived_diagnostics(diagnostics, display, PredictionDerivedStatus::ContinuityFailed);
                return false;
            }
            ...
            if (resolved_frame_spec.type == orbitsim::TrajectoryFrameType::Inertial)
            {
                clipped_segments = clipped_inertial_segments;
                chunk_diagnostics = make_stage_diagnostics_from_segments(
                        clipped_segments,
                        prediction_segment_span_s(clipped_inertial_segments));
            }
            else
            {
                const auto &base_ephemeris = solver.resolved_shared_ephemeris();
                ...
                const orbitsim::FrameSegmentTransformOptions frame_opt =
                        PredictionCacheInternal::build_frame_segment_transform_options(
                                ...
                                cancel_requested);
                orbitsim::FrameSegmentTransformDiagnostics frame_diag{};
                clipped_segments = orbitsim::transform_trajectory_segments_to_frame_spec(
                        clipped_inertial_segments,
                        *base_ephemeris,
                        base_bodies,
                        resolved_frame_spec,
                ...
                if (!validate_trajectory_segment_continuity(clipped_segments))
                {
                    update_derived_diagnostics(diagnostics, display, PredictionDerivedStatus::ContinuityFailed);
                    return false;
                }

                chunk_diagnostics = make_stage_diagnostics_from_adaptive(
                        frame_diag,
                        prediction_segment_span_s(clipped_inertial_segments));
                chunk_diagnostics.accepted_segments = clipped_segments.size();
                chunk_diagnostics.covered_duration_s = prediction_segment_span_s(clipped_segments);
                chunk_diagnostics.frame_resegmentation_count = frame_diag.frame_resegmentation_count;
            }

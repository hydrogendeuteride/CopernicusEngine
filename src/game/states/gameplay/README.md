...
gameplay/
  ...
  orbital_runtime_system.h/.cpp   # orbital simulation runtime ownership and scenario initialization
  orbital_physics_system.h/.cpp   # physics/orbitsim coupling, runtime rails, rails warp, formation hold
  gameplay_orbital_context.h/.cpp # GameplayState-to-OrbitalPhysicsSystem context adapter
  orbiter_world_state_provider.*  # shared orbiter world-state sampling for prediction/physics
...
- `orbital_runtime_system.h`
  `OrbitalRuntimeSystem` owns the runtime `OrbitalScenario`, orbiter registry, player orbiter selection, and scenario-to-orbitsim initialization through `initialize_scenario()`.

- `gameplay_orbital_context.h`
  `GameplayOrbitalContextBuilder` adapts `GameplayState` state into `OrbitalPhysicsSystem::Context`, keeping physics context assembly out of the main state API.

- `orbiter_world_state_provider.h`
  Shared helper for sampling an orbiter's world position, inertial velocity, and local velocity from either a physics body or rails state.
...
- `gameplay_state_sim.cpp`
  Time-warp mode transition glue. Heavy physics and rails behavior lives in `OrbitalPhysicsSystem`.

- `orbital_runtime_system.cpp`
  Orbital runtime implementation: scenario initialization, celestial body registration, orbiter registry lookup, player orbiter tracking, and runtime reset.

- `gameplay_orbital_context.cpp`
  Context adapter for fixed-update physics calls and rails transitions.

- `orbiter_world_state_provider.cpp`
  Physics/rails world-state sampling shared by prediction subject state lookup and orbital physics context callbacks.

- `orbital_physics_system.cpp`
  Simulation implementation: `step_physics()` (N-body gravity + physics world step), `enter_rails_warp()` / `exit_rails_warp()` / `rails_warp_step()` (on-rails time warp with thrust propagation), `update_runtime_orbiter_rails()` / `sync_runtime_orbiter_rails()` (automatic on/off-rails transitions by distance), `update_formation_hold()` (critically-damped spring LVLH station-keeping), `sync_celestial_render_entities()`.
...
- Physics stepping, gravity, runtime rails, or rails-warp behavior:
  Start in `orbital_physics_system.cpp`.

- Time-warp level transitions:
  Start in `gameplay_state_sim.cpp`.

- Scene setup, entity spawning, or orbitsim initialization:
  Start in `gameplay_state_scene.cpp` for entities and `orbital_runtime_system.cpp` for orbitsim scenario initialization.
...
- Runtime data for celestials, orbiters, or the orbital scenario:
  Start in `orbit_helpers.h` for data types and `orbital_runtime_system.h` for runtime ownership.
